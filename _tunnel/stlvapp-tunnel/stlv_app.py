...
from stelvio.aws.api_gateway import Api  # REST API (unused here but kept for reference)
...
@app.run
def run() -> None:
    # Keep existing REST tunnel example for reference
    incomingRequestApi = Api("incoming-request-api")
    incomingRequestApi.route("GET", "/tunnel/{channel_id}", "functions/incoming.handler")
