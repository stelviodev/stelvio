...
tagged with stelvio:env=test and stelvio:app starting with "stlv-".
...
# Services that are only reported, not auto-deleted
_REPORT_ONLY = {"cloudfront", "route53"}
...
def discover_by_tags(profile: str | None, regions: list[str]) -> list[DiscoveredResource]:
    """Find all resources tagged stelvio:env=test with stelvio:app starting with stlv-."""
    ...
    for region in regions:
        ...
        for page in paginator.paginate(TagFilters=[{"Key": "stelvio:env", "Values": ["test"]}]):
            for item in page.get("ResourceTagMappingList", []):
                ...
                if not app_tag.startswith("stlv-"):
                    continue
...
def _name_from_arn(arn: str) -> str | None:
    ...
    for prefix in ("function:", "layer:", "role/", "policy/", "table/", "rule/"):
...
def delete_resources(
    ...
    _report_only_resources(by_service)
...
def _report_only_resources(
    by_service: dict[str, list[DiscoveredResource]],
) -> None:
    for service in _REPORT_ONLY:
        if service in by_service:
            print(f"\n  [Report only] {service} — manual deletion required:")
...
def _run_deletion_phase(
    ...
    for r in phase_resources:
        ...
        try:
            ...
            print(f"    Deleted {r.service}: {r.name}")
            succeeded += 1
