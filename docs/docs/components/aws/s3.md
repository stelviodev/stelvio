...
Using the [linking mechanism](../../concepts/linking.md), you can easily access the S3 bucket in your Lambda functions using the regular [`boto3`](https://boto3.amazonaws.com/) library:
...
You can link other resources to your notification function using the `links` parameter. For details on how linking works and default permissions, see the [Linking guide](../../concepts/linking.md).
...
!!! warning "Queue and Topic Policy Behavior"
...
| `links`         | List of links to grant the notification function access to other resources. Optional.                      |
| `**opts`        | Additional function configuration options (`memory`, `timeout`, `environment`, `architecture`, `runtime`, `requirements`, `layers`, `url`). Only valid when `function` is specified as a string. These are unpacked from `FunctionConfigDict`.  |
...
| `filter_prefix` | Filter notifications by object key prefix. Optional.                                                       |
| `filter_suffix` | Filter notifications by object key suffix. Optional.                                                       |
| `topic`         | SNS topic to send notifications to. Can be a `Topic` component or topic ARN string. Optional.              |
...
| `subscriptions`            | List of `BucketNotifySubscription` components created via notification methods.                  |
...
In both cases, the files uploaded to your static website are considered part of your infrastructure. Thus, the files would be automatically deployed whenever you run `stelvio deploy`. However, in the latter case, uploaded files are not part of your (Pulumi) state and thus not tracked. 

Using the `stelvio_resources` module, you can access the S3 bucket and manage the files (assets) in your static website, should you decide that part should not be part of your deployment. Note that at the moment, you can get the `arn` of the bucket via `stvl_resources` only from within a Lambda function.

!!! note

    If you decide to upload your file assets manually, you must also take care of removing the files from the bucket before running `stelvio destroy`, as the AWS API does not allow deleting a non-empty S3 bucket.
...
If you want to expose a bucket along with other resources, such as an API Gateway, you can use the [`Router` component](cloudfront-router.md).
...
The `Bucket` component supports the `customize` parameter to override underlying Pulumi resource properties. For an overview of how customization works, see the [Customization guide](../../concepts/customization.md).

### Resource Keys

| Resource Key           | Pulumi Args Type                                                                                        | Description                            |
|------------------------|---------------------------------------------------------------------------------------------------------|----------------------------------------|
| `bucket`               | [BucketArgs](https://www.pulumi.com/registry/packages/aws/api-docs/s3/bucket/#inputs)                   | The S3 bucket itself                   |
| `public_access_block`  | [BucketPublicAccessBlockArgs](https://www.pulumi.com/registry/packages/aws/api-docs/s3/bucketpublicaccessblock/#inputs) | Public access block settings |
| `bucket_policy`        | [BucketPolicyArgs](https://www.pulumi.com/registry/packages/aws/api-docs/s3/bucketpolicy/#inputs)       | Bucket policy (when `access="public"`) |
| `bucket_notification`  | [BucketNotificationArgs](https://www.pulumi.com/registry/packages/aws/api-docs/s3/bucketnotification/#inputs) | Bucket notification configuration |
| `subscriptions`        | (nested `BucketNotifySubscriptionCustomizationDict`)                                                    | Notification subscription resources    |
| `function`             | [BucketNotificationLambdaFunctionArgs](https://www.pulumi.com/registry/packages/aws/api-docs/s3/bucketnotification/#bucketnotificationlambdafunction) | Lambda notification config block |
| `queue`                | [BucketNotificationQueueArgs](https://www.pulumi.com/registry/packages/aws/api-docs/s3/bucketnotification/#bucketnotificationqueue) | SQS queue notification config block |
| `topic`                | [BucketNotificationTopicArgs](https://www.pulumi.com/registry/packages/aws/api-docs/s3/bucketnotification/#bucketnotificationtopic) | SNS topic notification config block |
...
| Resource Key             | Pulumi Args Type                                                                            | Description                                    |
|--------------------------|--------------------------------------------------------------------------------------------|------------------------------------------------|
| `bucket`                 | Nested (see [Bucket customization](#bucket) above)                                          | The S3 bucket                                  |
| `files`                  | [BucketObjectArgs](https://www.pulumi.com/registry/packages/aws/api-docs/s3/bucketobject/#inputs) | Uploaded files from the directory              |
| `cloudfront_distribution`| Nested (see [CloudFrontDistribution customization](cloudfront-router.md#cloudfrontdistribution)) | CloudFront distribution                        |
