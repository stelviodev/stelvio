...
- **`DynamoTable` global index deprecation warnings.** Global secondary indexes no longer print `hashKey is deprecated` / `rangeKey is deprecated` on every deploy and diff. They now use the provider's `key_schema` form. Existing tables see no diff, because the key schema is identical.
- **`Bucket` versioning deprecation warning.** Buckets no longer print `versioning is deprecated`. Versioning is now a separate `BucketVersioning` resource, created only when `versioning=True`. Existing buckets see no diff. Setting `versioning=False` on a versioned bucket suspends versioning, as S3 can't make a bucket unversioned again.
