...
uv run pytest "$INTEGRATION_DIR" --integration $COMMON_ARGS -n 10 -p no:warnings &
pids+=($!)

# CloudFront tier — 7 workers for 13 tests (slow teardown, mostly waiting on AWS)
uv run pytest "$INTEGRATION_DIR" --integration-cf $COMMON_ARGS -n 7 -p no:warnings &
pids+=($!)

# DNS tier — only if domain env vars are set
if [[ -n "${STLV_TEST_DNS_DOMAIN:-}" && -n "${STLV_TEST_DNS_ZONE_ID:-}" ]]; then
    uv run pytest "$INTEGRATION_DIR" --integration-dns $COMMON_ARGS -n 4 -p no:warnings &
    pids+=($!)
fi
