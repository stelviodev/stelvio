...
You can subscribe to S3 bucket events using the `notify` method. Event notifications can trigger either Lambda functions, SNS topics, or SQS queues when objects are created, deleted, or modified.
...
!!! warning "Queue and Topic policy behavior"
    When using a Queue or Topic as a notification target, Stelvio creates an SQS QueuePolicy or SNS TopicPolicy resource to allow S3 to send messages. These policy resources **replace** any existing policy on the queue or topic. If you have custom policies on your queues or topics, you may need to manage permissions manually.

#### Topic Notifications

Publish event notifications to an SNS topic for fan-out to multiple subscribers:
...
!!! note "External Queues/Topics Policy"
    When using a `Queue` or `Topic` component as a notification target, Stelvio creates an SQS `QueuePolicy` or SNS `TopicPolicy` resource to allow S3 to publish notifications. These policy resources **replace** any existing policy on the queue or topic.

    When using an ARN string (external queue/topic), Stelvio does **not** create or manage a policy resource. You are responsible for ensuring the queue/topic policy allows S3 to send notifications from the bucket.

!!! note "Topic notifications don't support links"
    The `links` parameter cannot be used with `queue` or `topic` notifications, as these resources do not execute code directly. If you need to grant permissions, add links when subscribing a function to the queue or topic.
...
| `links`         | List of links to grant the notification function access to other resources. Only for function targets.     |
| `**opts`        | Additional function configuration options (memory, timeout, etc.) when function is specified as a string.  |

!!! note "Exactly one target required"
    You must specify exactly one target for the notification: `function`, `queue`, or `topic`.
...
| `subscriptions`            | List of `BucketNotifySubscription` components created via `Bucket.notify()`.                     |
