...
| Parameter    | Description                                                                        |
| ------------ | ---------------------------------------------------------------------------------- |
| `versioning` | The versioning configuration for the S3 bucket. Boolean. Default is `False`. Setting it back to `False` on a bucket that was versioned *suspends* versioning rather than removing it, because S3 can't return a bucket to an unversioned state. |
