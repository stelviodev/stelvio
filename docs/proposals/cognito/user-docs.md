...
Stelvio supports Amazon Cognito for user authentication using the `UserPool` component. You can create user pools with email/password sign-in, social login providers, MFA, and Lambda triggers — with Stelvio handling the IAM wiring automatically.
...
| `custom_email_sender` | Use a third-party email service instead of SES |
| `custom_sms_sender` | Use a third-party SMS service |
...
!!! note "Pricing Changes Over Time"
    Cognito pricing changes occasionally and can vary by region. Always verify current pricing in the [AWS Cognito pricing page](https://aws.amazon.com/cognito/pricing/).
