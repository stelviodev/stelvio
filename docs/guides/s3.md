...
!!! note "Topic notifications don't support links"
    Unlike function notifications, topics don't execute code directly. The `links` parameter cannot be used with topic notifications. If you need to grant permissions to subscriber functions, add links when subscribing to the topic.
...
| `topic_policies`           | List of SNS `TopicPolicy` resources allowing S3 to publish messages to notification topics.      |
