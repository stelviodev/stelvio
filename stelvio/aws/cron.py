...
class Cron(Component[CronResources]):
    ...
    def __init__(
        ...
        **opts: Unpack[FunctionConfigDict],
        ...
        self._handler_config = self._parse_handler(handler, opts)
    ...
    def _validate_schedule(self) -> None:
        ...
        if schedule.startswith("rate("):
            self._validate_rate_expression(schedule)
    ...
    def _validate_rate_expression(schedule: str) -> None:
        ...
        if len(parts) != expected_parts:
            raise ValueError(
                f"Invalid rate expression: expected 'rate(value unit)', got: {schedule}"
            )
    ...
    def _get_or_create_function(self) -> Function:
        """Get existing Function or create new one from config."""
        if isinstance(self._handler_config, Function):
            return self._handler_config
        return Function(f"{self.name}-fn", config=self._handler_config)

    def _create_resources(self) -> CronResources:
        stelvio_function = self._get_or_create_function()
