...
def test_websocket_api_custom_domain_mappings_and_connection(
    ...
    assert_apigatewayv2_execute_endpoint(
        outputs["websocket_api_sharedwebsocket_id"], disabled=True
    )
    ...
    websocket_connect(
        outputs["websocket_api_customwebsocket_url"],
        retry_timeout=_WEBSOCKET_DNS_CONNECT_TIMEOUT,
    )
    websocket_connect(
        outputs["websocket_api_sharedwebsocket_url"],
        retry_timeout=_WEBSOCKET_DNS_CONNECT_TIMEOUT,
    )
