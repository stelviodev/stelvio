...
class AcmValidatedDomain:
    ...
    def create(self):
        # 1 - Issue Certificate
        self.certificate = pulumi_aws.acm.Certificate(self.prefix_fn(f"{self.name}-custom-domain-certificate"),
        ...
        self.cert_validation = pulumi_aws.acm.CertificateValidation(self.prefix_fn(f"{self.name}-custom-domain-certificate-validation"),
            certificate_arn=self.certificate.arn,
            validation_record_fqdns=[self.validation_record.name],  # This ensures validation_record exists
            opts= pulumi.ResourceOptions(depends_on=[self.certificate, self.validation_record._pulumi_resource]))
